...
Environment assessment produces `EnvironmentAssessment` (§10.2.25): credentials, Docker/toolkit, GPU memory + compute capability, deployable models, placement-aware embedding availability, and active Blueprint-managed NIM residents. Hardware gates eligibility; a curated evidence-backed quality rank selects Omni on supported ≥80 GB / cc≥9.0 GPUs, CR3-Nano at ≥56 GB when Omni is ineligible, and CR2-2B at 36–55 GB. Memory-floor checks allow a 1% `nvidia-smi` reporting tolerance so a nominal 80 GB card reported near 79.6 GiB remains in its documented tier. Super and CR2-8B remain selectable. An exact healthy Teacher resident outranks a hosted key only when it matches that recommendation: fresh-project creation attaches its endpoint and selects that model before returning. A different resident remains visible for an explicit keep/replace choice. Without a preferred resident, a fitting local Teacher is recommended for lower latency; an API key supplies an immediate hosted bridge during download, and the project activates local atomically only after verification. Hosted-only remains available. GPUs in the 10–35 GB tier recommend hosted Teacher + local NeMo Retriever VL embeddings; below 10 GB, embeddings are hosted or pHash.
...
After backend startup, service connection setup remains in the web app (NIM Connection screen on first project creation).
...
**Pool assignment routing** — Automatic routing of newly Verified examples to the Test Pool or non-pool status based on a configured fraction and CLIP-embedding diversity. Verification outcome (Accept/Edit) does not influence pool assignment (§4.3.1–4.3.2).
...
**pHash (perceptual hash)** — A compact 64-bit perceptual fingerprint computed inline at ingest (CPU-only, no external dependency). Similarity measured by hamming distance. Baseline diversity signal for the review selector. See §5.6.

**pHash-diverse review selector** — Baseline selection strategy using pHash hamming distance. Always available because pHash is computed inline at ingest. Deterministic given persisted scheduler state.
...
2. Student Training execution is validated server-side independently of the Scale-Up Readiness Gate: fine-tuning cannot start until TAO/workspace and training-data checks pass and each selected base model has the `student_base` role. A selected missing TAO base is non-blocking because first-use provisioning is the first conditional Training Jobs stage.
...
Example record minimum fields: `project_id`, `example_key` (unique within project), `storage_ref` (absolute filesystem path), `ingested_at`, `source_metadata` (JSON, may be `{}`), `state`. Omission provenance (required when `state=Omitted`, null otherwise): `omitted_source` ∈ {`sme_skip`, null}, `omitted_at`. pHash field required: `phash` (hex-encoded, computed inline at ingest, §5.6). Embedding fields: `clip_embedding_present` (default false), `clip_embedding_dim`, `clip_embedding_model_id`, `embedding_provider`. Prior-label reference fields (set during semantic Core schema change, null otherwise): `prior_verified_label_ref` (JSON snapshot of prior label data), `prior_verified_outcome` (`Accept` / `Edit` / null). System MUST NOT assume embeddings exist for all examples. Omission is set by SME Skip; `omitted_source` and `omitted_at` MUST be recorded automatically. See §4.2 for full field list.
...
- Model configs include image input capability:
  ...
  - Seeding: Cosmos Reason2 8B/2B, Mistral Large 3, Nemotron Nano VL, Nemotron 3 Nano Omni → `true`.
...
- `mistralai/mistral-large-3-675b-instruct-2512` (262144) | teacher | vision: yes | media: `none` | *operator-selected alternate*
- `nvidia/nemotron-nano-12b-v2-vl` (128000) | teacher | vision: yes | media: `mm_processor_tiles`
- `stepfun-ai/step-3.7-flash` (262144) | teacher | vision: yes | media: `none` | *alternate — replaced as default 2026-07-23 after a latency regression; `always_on_reasoning`; image cap 8*
- `minimaxai/minimax-m3` (500000 live-probed floor) | teacher | vision: yes | media: `none` | ***default Teacher*** *(selected 2026-07-23; deep-ICL ceiling 8 — only hosted Teacher usable past 32 images; ~5 MB body cap binds first)*
...
Create Guidance (new immutable version). Ingest images (Example records: `storage_ref`, `ingested_at`, `source_metadata`). pHash computed inline. CLIP embedding computation triggered in background. Cold start supported: projects may begin with `Verified = 0`. See §6.1.
...
- **Per-model depth default (`ModelConfig.default_icl_max_examples: int | null`, §6.2):** effective selection cap = explicit `icl_max_examples` override (per-run API field or non-null `ICL_MAX_EXAMPLES`) when present — the override wins in either direction; else the model default; else uncapped. Resolved centrally in `prompt_service.invoke_teacher` so proposal/eval/batch cannot drift. Adaptive-K trims within the cap; token/image pruning still applies after. Seeded (July 2026 depth studies): Nemotron Nano VL 2 · Nemotron 3 Nano Omni 4 · CR3 nano/super 8 · CR2-2B 8 · CR2-8B 16 · MiniMax-M3 8 · Mistral Large 2 · null for unmeasured entries.
...
- **Configuration snapshot** on Run Record: `teacher_model_config_id`, `active_guidance_id`, `labeling_generation_preset_key`, `thinking_default_on`, `visual_budget_preset_key`, effective Inference Contract (§6.11), `icl_eligible_count_at_start`. Config changes mid-run MUST NOT affect the in-flight run.
...
**3. Minimum per-value F1.** Every value of every categorical Core field (enum, enum_set, boolean) MUST have F1 meeting `SCALEUP_MIN_PER_VALUE_F1_THRESHOLD` (default 0.80). Catches systematic misclassification even when overall field metrics pass. Reports failing values with F1 + precision + recall (diagnostic context).
...
3. **Snapshot config** on Run Record: `teacher_model_config_id`, `active_guidance_id`, `labeling_generation_preset_key`, `thinking_default_on`, `visual_budget_preset_key`. All steps use snapshotted values. Mid-run config changes MUST NOT affect the in-flight run.
...
At least one base must remain selected. See §9.4.
...
- **Serving validation (NIM-backed):** NIM deployment enables latency benchmarks, throughput, profile metadata. Local orchestration via Docker (§9.5.2); Action Request fallback if preflight fails. Flips `serving_status` and records `serving_evaluation_run_id` independently of the quality path.
...
System attempts to deploy Student NIM containers locally for evaluation. Temporary infrastructure, not production.

**Preflight (6 checks):** (1) Docker available, (2) NVIDIA Container Toolkit (GPU passthrough), (3) GPU memory sufficient (BF16: 8B >56GB, 2B >36GB; FP8 uses model-specific `nim_gpu_memory_minimum_gb`), (4) NGC_API_KEY valid, (5) NIM container image pullable, (6) checkpoint exists with `checkpoint_packaging_status="validated"`. Persisted: `nim_preflight_status`, `nim_preflight_details`, `nim_preflight_at`.

**If preflight passes (Tier 1 local orchestration):** **(0) acquire GPU (F49 amendment, 2026-05-19)** — stop every `LocalNimDeployment` row in `starting | running` on the target GPU before docker_run; each displaced row's `displaced_by_deployment_id` (the Student's deployment_id) + `displaced_at` are persisted on §13.15; on multi-GPU hosts where auto-placer found a free device, step 0 is a no-op; (1) construct `docker run` from StudentModel (checkpoint path, served model name, NIM cache, NGC key, port, `--shm-size=32GB`, `-u $(id -u)`, GPU allocation); (2) start, poll `/v1/health/ready` up to `NIM_STARTUP_TIMEOUT_S` (default 1200s); (3) smoke inference; (4) register temporary endpoint as ModelConfig; (5) run NIM evaluation against Test Pool with `evaluation_source="nim"`; (6) latency benchmarks at `STUDENT_LATENCY_TEST_CONCURRENCIES` (default [1,8,24]), collect Prometheus metrics if available; (7) stop container (persistent NIM cache retained for next startup); (8) repeat next variant; **(9) auto-restore displaced residents (F49 amendment, 2026-05-19)** — iterate step 0's displaced deployments and best-effort re-deploy each (same role / model_config_id / gpu_assignment), health-poll up to `NIM_STARTUP_TIMEOUT_S`; failure surfaces as warning on `serving_evaluation_run_id` summary but does NOT fail `serving_status`. Container lifecycle events logged on StudentModel.

**If preflight fails (Tier 2 Action Request fallback):** generate `student_nim_deploy` Action Request (§10.3) with exact `docker run` command, checkpoint path, GPU requirements, NIM release, env vars, mount paths, health check, smoke test, temporary-infra note. SME hands to infrastructure; once endpoint is running, SME registers URL; system runs evaluation + benchmarks against that endpoint.
...
The optional diagnostics endpoint (§10.2.22) reports the same readiness checks
that **Start Training** enforces internally; it is not part of the SME UI.

**Per-training dataset upload (§9.7.8.2, Blueprint-automated):** before submitting any TAOJob, upload each training suite's training + Test Pool evaluation archive to the workspace's S3 bucket at `s3://{bucket}/vlm-feedback-loop/projects/{project_id}/dataset_exports/{dataset_export_id}/{archive_name}`. Multipart upload ≥ 8 MiB. Idempotent per `(workspace_id, dataset_export_id)`. Persists S3 URI on DatasetExport (`dataset_upload_ref`, `dataset_upload_uri`). TAOJob `tao_create_job_request.specs` reference uploaded S3 paths (training) or dataset IDs (evaluation), NOT Blueprint host's local filesystem. Upload failures surface as 409 `tao_dataset_upload_failed` with retriable hint.

**Preflight additions (§9.7.8.3):** `tao_workspace_reachable` (bootstrap_status=bootstrapped AND `GET /orgs/{org}/workspaces/{workspace_id}` returns 200) and `tao_base_experiment_ready` (non-null UUID AND pull status `pull_complete`). Failure messages reference both provisioning paths.
...
- **Model catalog:** `POST/GET/LIST/PATCH /v1/projects/{project_id}/model_configs` (§10.2.19, role filtering; **F22 amendment 2026-05-05** — GET response exposes `tao_base_experiment_id` and `tao_base_experiment_pull_status` as read-only fields so operators can verify TAO provisioning state without direct sqlite3 access; PATCH continues to reject both as `extra_forbidden`); `POST .../model_configs/{model_config_id}:reprobe` (§10.2.12, 409 when in active use by running run/job)
- **Student models:** `GET .../student_models` + `.../{id}` (§10.2.20); `POST .../student_models/{id}:deploy_nim` (local Tier 1 or external Tier 2)
- **Training readiness and counts:** `POST
  ...
  authoritative Verified Training Pool, Test Pool, Auto-Labeled
  eligible/included, excluded, and final usable counts.
- **TAO jobs:** `POST/GET/LIST /v1/projects/{project_id}/tao_jobs` (§10.2.7); `POST .../tao_jobs/{tao_job_id}:cancel?force_local=<bool>` (transitions to `canceled`, halts downstream `not_started` siblings with `chain_halted_reason`; **F17 amendment 2026-05-05** — `force_local=true` skips the TAO POST entirely so a job whose `tao_external_job_id` was orphaned by a TAO server rebuild can still cancel locally; the canceled row's `poll_error_ref` is stamped `forced_local_cancel: external TAO unreachable or external_id orphaned` for audit, and a `warning`-level log line is emitted on every use); training suite endpoint `POST/GET/LIST /v1/projects/{project_id}/training_suites` (Phase 11 Step 11.2, idempotency key; exports build+commit in worker-thread sessions per §1.8 write discipline, chain + suite rows insert in one transaction). `POST .../training_suites/{training_suite_id}:cancel` terminalizes the suite first, best-effort cancels setup plus every remaining remote job, locally cancels all remaining jobs even when TAO cannot confirm, and returns the remote failures for operator warning; late workers MUST NOT reactivate the suite or advance a chain. **Training-suite response top-level keys** (canonical names — F2 amendment 2026-05-05; the lifecycle key is `status`, NOT `suite_status`): `training_suite_id`, `project_id`, `idempotency_key`, `guidance_id`, `training_preset`, `export_field_mode`, `include_auto_labeled`, `quantization_schemes`, `training_dataset_export_id`, `evaluation_dataset_export_id`, `selected_student_base_model_config_ids`, `chain_ids_ordered`, `chains[]` (each: `chain_id`, `student_base_model_config_id`, `base_model_name`, `jobs[]` — each: `tao_job_id`, `action`, `chain_sequence`, `status`, `tao_external_job_id`, `chain_halted_reason`), `status`, `created_at`, `started_at`, `completed_at`. `chains[].jobs[]` is eagerly populated by `_build_chains_snapshot` inside the read session — F1 paraphrase observed empty `chains[]` was non-reproducible against current code.
...
- **Review selector:** if CLIP embeddings missing/unavailable, selection MUST use pHash-diverse mode (§5.6). pHash is always available for successfully ingested images.
...
- (c) **Persisted to .env** — `POST /v1/secrets:set` with `persist=true` AND deployment-level `ALLOW_UI_SECRET_PERSIST=true` upserts the line in `~/.vlm_feedback_loop/.env` (`0600` perms), reloads `Settings`, and clears the runtime override. Container/production deployments set `ALLOW_UI_SECRET_PERSIST=false` to disable this path; endpoint returns 403 `ui_secret_persist_disabled`.
...
- Filesystem operations (§10.2.10) MUST enforce one `IMAGE_ROOT`. When the backend is network-accessible (non-loopback bind), it MUST be explicitly configured. Symlinks escaping it MUST be rejected. The initial browse request omits `path`, so the backend opens the effective root automatically; an unset loopback root means `/`.

---

## 13. Required Records  `[REFERENCE]`

19 record families persist all domain data. Full schemas with minimum field lists, constraints, and notes are in `Engineering_Spec.md §13.1–§13.19`. The state machines (§13.2.1, §13.2.2) are kept EXACT below because they are cited cross-repo; all other record families are referenced.
...
| 13 | §13.13 | **StudentModel** | Trained/deployed Student with training lineage, `checkpoint_packaging_status`, `quality_status` (4-state F35: `pending`/`validated`/`partial`/`failed`), `serving_status`, NIM deployment state, `quantization_method`. |
...
Additional batch labeling run fields: `paused_reason`, `examples_succeeded`, `examples_schema_invalid`, `examples_timeout`, `examples_endpoint_error`, `examples_total`.
...
**Prompt budgets:** `RUNTIME_PROMPT_TOKEN_SAFETY_MARGIN: 0.85`, `BASE_OUTPUT_TOKENS_FLOOR: 256`, `JSON_STRUCTURAL_OVERHEAD_TOKENS: 48`, `MAX_OUTPUT_FRACTION: 0.25`, `RATIONALE_NOTE_ESTIMATE_TOKENS: 160`, `DEFAULT_UNBOUNDED_STRING_BUDGET: 200`, `MODEL_REASONING_HEADROOM_TOKENS: 16384`.

**ICL:** `ICL_MAX_EXAMPLES: null`.

**Export:** per-project `export_field_mode: all`.

**Test Pool:** per-project `test_pool_fraction: 0.40` (Project record §13.4).

**Scale-Up Gate:** per-project settings (Project record §13.4): all thresholds 0.80 (`scaleup_exact_match_threshold`, `scaleup_per_field_match_threshold`, `scaleup_min_per_value_f1_threshold`, `scaleup_accept_rate_threshold`); `scaleup_accept_rate_window: 50`; `scaleup_min_test_pool_size: 60`.

**Batch labeling:** `BATCH_LABEL_RUN_LIMIT: null`, `BATCH_LABEL_CIRCUIT_BREAKER_THRESHOLD: 10`, `BATCH_LABEL_CONCURRENCY_HOSTED: 1`, `BATCH_LABEL_CONCURRENCY_SELF_HOSTED: 8`.
...
- **Full-stack validation (Phase 12.4)** — C20 (handoff `docker run` re-execution proof for both 2B and 8B), C21 (2B vs 8B handoff content differentiation), final integration checkpoint, Cosmos-RL format validator, `LOCAL_NIM_MOCK_ENDPOINT_URL` test seam. Mock-validated in `tests/unit/test_full_stack_validation_mock.py`; live-validated via a rental-window operator runbook (retained in the project's internal engineering archive).
- **Backend type checking (Phase 13 Step 13.2 — COMPLETE)** — 4 verify items for `[tool.pyright]` strict-mode adoption. Mirrors Retail-Agentic-Commerce. **Step legitimately spanned multiple PRs** because strict pyright surfaced 1685 errors against `main` (well above the 200-error threshold for the multi-PR fallback). The full PR sequence (foundation → bare-collection parametrization → mechanical cleanup → narrowing → real-bug fixes → CI gate) and per-PR audit are documented in the project's internal engineering archive. All 4 verify items PASS as of PR-7's merge: pyright clean (0 errors), `backend-typecheck` CI job is the 6th workflow job, total `# pyright: ignore[code]` count = 20 (at the hard cap, mirroring Retail-Agentic-Commerce's 16 footprint).
...
- **E.3** Concurrency Scaling / SLO Miss Rate (NIM via `genai-perf`)
- **E.4** Efficiency Normalization
- **E.5** Robustness (Long Input / Long Output)

Apples-to-apples rules, required provenance per run (`student_model_config_id`, `nim_model_profile_*`, `quantization_method`, GPU model/count, dataset manifest SHA-256, identical decoding params across variants, identical `visual_budget_preset_key`). See `Engineering_Spec.md Appendix E`.
