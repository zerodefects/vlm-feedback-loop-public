...
| `export_public_snapshot.py` | Export a committed, curated public-repository snapshot; source-host-only agent instructions and AutoRun are excluded by default, while AutoRun can be restored with `--include-autorun` |
| `generate_third_party_licenses.py` | Regenerate `LICENSE-3rd-party.txt` after dependency changes |
| `render_architecture_diagram.py` | Re-render `docs/images/architecture.png` from the `.mmd` source with an inline uv-managed tool environment; pass `--check` for a pixel-level drift gate |
...
| Script | Gate | Dataset |
|---|---|---|
| `tao_live_smoke.py` | Fast TAO wiring: provisioning/discovery → train → LoRA merge → local Student NIM baseline evaluation → checkpoint/lineage verification | Three generated images; a Student quality failure is expected and is not this smoke's pass criterion |
